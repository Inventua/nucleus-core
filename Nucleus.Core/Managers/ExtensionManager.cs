...
namespace Nucleus.Core.Managers
{
  ...
  public class ExtensionManager : IExtensionManager
  {
    ...
    public ExtensionManager(IDataProviderFactory dataProviderFactory, IPageModuleManager pageModuleManager, IScheduledTaskManager scheduledTaskManager, IOptions<Nucleus.Abstractions.Models.Configuration.FolderOptions> folderOptions, ILogger<IExtensionManager> logger)
    {
    ...
    /// <returns></returns>
    public async Task DeleteScheduledTasks(string assemblyFilePath)
    {
      System.Reflection.Assembly assembly = System.Reflection.Assembly.LoadFile(assemblyFilePath);

      foreach (Abstractions.Models.TaskScheduler.ScheduledTask task in await this.ScheduledTaskManager.List())
      {
        if (assembly.GetType(task.TypeName.Split(',').First(), false, true) != null)
        {
          // the scheduled task is implemented by the assembly
          await this.ScheduledTaskManager.Delete(task);
        }
      }
    }
    ...
    public async Task DeleteModuleDefinition(ModuleDefinition moduleDefinition)
    {
      ...
      foreach (PageModule module in await this.ListPageModules(moduleDefinition))
      {
        await this.PageModuleManager.Delete(module.Id);
      }
