	...
	public class AssemblyLoader
	{
		...
		public static Assembly LoadFrom(string path)
		{
			...
			else
			{
				...
				{
					//assemblyLoadContext = new System.Runtime.Loader.AssemblyLoadContext(extensionFolder, true);
					assemblyLoadContext = new PluginLoadContext(pluginPath);
		...
		{
			System.IO.DirectoryInfo workingDirectory = new System.IO.DirectoryInfo(fullPath);
			while (workingDirectory.Parent != null)
			{
				if (workingDirectory.Parent.Name.Equals(Folders.EXTENSIONS_FOLDER, StringComparison.OrdinalIgnoreCase))
				{
		...
		private static Assembly HandleResolving(AssemblyLoadContext context, AssemblyName assemblyName)
		{
			if (context == AssemblyLoadContext.Default)
			{
				if (AssemblyLoadContext.CurrentContextualReflectionContext != null)
				{
					...
					//foreach (Assembly asm in AssemblyLoadContext.CurrentContextualReflectionContext.Assemblies)
					//{
					//	if (asm.FullName == assemblyName.FullName)
					//	{
					//		return asm;
					//	}
					//}
				...
				else
				{
					foreach (AssemblyLoadContext moduleContext in ExtensionLoadContexts.Values)
					{
						...
						//foreach (Assembly asm in moduleContext.Assemblies)
						//{
						//	if (asm.FullName == assemblyName.FullName)
						//	{
						//		return asm;
		...
		{
			foreach (string path in System.IO.Directory.GetDirectories(Nucleus.Abstractions.Folders.GetExtensionsFolder()))
			{				
		...
		private static IEnumerable<string> EnumerateAssemblies()
		{
			...
			results.AddRange(System.IO.Directory.EnumerateFiles(Folders.GetExtensionsFolder(), "*.dll", System.IO.SearchOption.AllDirectories));
		...
		/// A side effect of this function is that assemblies are loaded into their assembly load contexts by <see cref="LoadAssemblies"/>.
		/// </remarks>
		internal static IEnumerable<Assembly> GetAssembliesImplementing<T>()
		{
