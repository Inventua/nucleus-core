...
[System.ComponentModel.DisplayName("Nucleus Forums: Send Forum Emails (summary)")]
public class SendSummaryForumEmailsScheduledTask : IScheduledTask
{
  ...
  private ILogger<SendSummaryForumEmailsScheduledTask> Logger { get; }

  public SendSummaryForumEmailsScheduledTask(ForumsManager forumsManager, IUserManager userManager, ISiteManager siteManager, IPageManager pageManager, IPageModuleManager pageModuleManager, IMailClientFactory mailClientFactory, IMailTemplateManager mailTemplateManager, ILogger<SendSummaryForumEmailsScheduledTask> logger)
  {
  ...
  private async Task SendMessages(IProgress<ScheduledTaskProgress> progress, CancellationToken cancellationToken)
  {
    ...
    this.Logger.LogInformation("Sending forum messages (summary).");

    var queue = (await this.ForumsManager.ListMailQueue(NotificationFrequency.Summary))
    ...
    foreach (var group in queue)
    {
      ...
      if (template != null && !String.IsNullOrEmpty(emailAddress?.Value))
      {
        foreach (var moduleGroup in group.GroupedItems.GroupBy(item => item.ModuleId))
        {
          ...
          model.Forums.Add(new Models.MailTemplate.ForumSummary(forum, posts, replies));
        }
        ...
        model.ForumNames = String.Join(',', model.Forums.Select(forum => forum.Name));
  ...
  private async Task TruncateMailQueue(IProgress<ScheduledTaskProgress> progress, CancellationToken cancellationToken)
  {
    try
    {
      ...
      this.Logger?.LogInformation("{rows} old records were removed from the mail queue.", rows);
    }
    catch (Exception ex)
    {
