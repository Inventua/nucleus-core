...
Nucleus Extension are .Net Core ASPNET MVC projects which use a [controller](https://docs.microsoft.com/en-us/aspnet/core/tutorials/first-mvc-app/adding-controller) class which contains
actions (functions) which populate view model classes, which are passed to Razor views to generate output.
...
[Learn more about database scripts](/developers/database-scripts/).
...
and ==ChangedBy== columns because they are provided by `ModelBase`.
...
The settings class is a POCO (Plain old CLR object) which contains properties to represent the available settings for your extension.  If your module will have configuration 
...
store data in your module's database tables.  [More Information on Saving Settings](/developers/saving-settings).\
\
...
license.md, license` or `license.htm` are automatically displayed to end users during installation.  [More Information](/developer/packaging/).
