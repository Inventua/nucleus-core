...
public class CoreDataProvider : Nucleus.Data.EntityFramework.DataProvider, ILayoutDataProvider, IUserDataProvider, IPermissionsDataProvider, ISessionDataProvider, IMailDataProvider, IScheduledTaskDataProvider, IFileSystemDataProvider, IListDataProvider, IContentDataProvider, IApiKeyDataProvider, IOrganizationDataProvider, IExtensionsStoreDataProvider, ISearchIndexHistoryDataProvider
{
  ...
  /// <summary>
  /// Save the specified <paramref name="page"/>, including page routes.  Permissions must be saved separately.
  /// </summary>
  ...
  /// <summary>
  /// Save the specified <paramref name="module"/>, including module settings.  Permissions must be saved separately.
  /// </summary>
  /// <param name="pageId"></param>
  /// <param name="module"></param>
  /// <returns></returns>
  ...
  {
    Permission existing = await this.Context.Permissions
      .Where(existing => (existing.Id != Guid.Empty && existing.Id == permission.Id) || (existing.RelatedId == permission.RelatedId && existing.Role.Id == permission.Role.Id && existing.PermissionType.Id == permission.PermissionType.Id))
      .FirstOrDefaultAsync();
    ...
    {
      existing.AllowAccess = permission.AllowAccess;
  ...
  public async Task<List<File>> SearchFiles(Site site, string searchTerm)
  {
