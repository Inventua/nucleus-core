...
namespace Nucleus.Modules.Account.Controllers
{
	...
	public class LoginController : Controller
	{
		...
		public LoginController(Context context, ILogger<LoginController> Logger, UserManager userManager, SessionManager sessionManager, PageManager pageManager, PageModuleManager pageModuleManager, IOptions<ClaimTypeOptions> claimTypeOptions)
		{
		...
		public ActionResult Index(string returnUrl, string token)
		{
			...
			{
				return View("ResetPassword", new ViewModels.ResetPassword { ControlId = $"_{Guid.NewGuid().ToString("N")}", PasswordResetToken = token });
			}
			else
			{
		...
		async public Task<ActionResult> Login(ViewModels.Login viewModel)
		{
			...
			else
			{
				...
				{
					UserSession session = this.SessionManager.CreateNew(this.Context.Site, loginUser, viewModel.AllowRememberMe && viewModel.RememberMe, ControllerContext.HttpContext.Connection.RemoteIpAddress);
