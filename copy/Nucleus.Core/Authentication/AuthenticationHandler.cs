...
namespace Nucleus.Core.Authentication
{
	public class AuthenticationHandler : Microsoft.AspNetCore.Authentication.SignInAuthenticationHandler<AuthenticationOptions>
	{
		...
		/// </summary>
		public const string DEFAULT_AUTH_SCHEME = "Nucleus";
		...
		/// </remarks>
		public const string COOKIE_NAME = "Nucleus_Session";
		...
		public AuthenticationHandler(SessionManager sessionManager, UserManager userManager, LinkGenerator linkGenerator, SiteManager siteManager,
																 Context context,
		...
		protected override Task<AuthenticateResult> HandleAuthenticateAsync()
		{
			...
			if (!String.IsNullOrEmpty(sessionId))
			{
				...
				userSession = this.SessionManager.Get(Guid.Parse(sessionId));
				...
				if (userSession != null)
				{
					...
					if (!userSession.RemoteIpAddress.Equals (this.Context.Connection.RemoteIpAddress))
					{
						...
						this.SessionManager.Delete(userSession);
					...
					if (userSession.ExpiryDate > DateTime.UtcNow)
					{
						if (userSession.SlidingExpiry)
						{
							...
							this.SessionManager.Save(userSession);
			...
			if (userSession != null)
			{
				...
				user = this.UserManager.Get(this.SiteManager.Get(userSession.SiteId), userSession.UserId);
				...
				else
				{
					...
					{
						Logger.LogTrace("User Id {0} Adding Claim {1}.", userSession.UserId, SYSADMIN_CLAIMTYPE);
						claims.Add(new Claim(SYSADMIN_CLAIMTYPE, ""));
					}
					...
					identity = new ClaimsIdentity(claims, DEFAULT_AUTH_SCHEME);
				}				
			}				
			else
			{
			...
			return Task.FromResult(AuthenticateResult.Success(new AuthenticationTicket(principal, DEFAULT_AUTH_SCHEME)));
		}
		...
		protected override Task HandleChallengeAsync(AuthenticationProperties properties)
		{
			...
			{
				this.Context.Response.WriteAsync("Session expired.");
			}
			else
			{
				if (this.CurrentContext.Site != null)
				{
					...
					{
						Page loginPage = this.Context.RequestServices.GetService<PageManager>().Get(sitePage.LoginPageId.Value);
						if (loginPage != null)
						{
				...
				else
				{
					// use default login page
					this.RedirectToDefaultLogin();
					//this.Context.Response.Redirect($"/User/Account?returnUrl={System.Uri.EscapeUriString(this.Context.Request.Path)}");
				}
			}
		...
		protected override Task HandleSignInAsync(ClaimsPrincipal user, AuthenticationProperties properties)
		{
			...
			AppendCookie(user.FindFirstValue(SESSION_ID_CLAIMTYPE), properties);
			return Task.CompletedTask;
		}
		...
		protected override Task HandleSignOutAsync(AuthenticationProperties properties)
		{
			...
			if (!String.IsNullOrEmpty(sessionId))
			{
				...
				UserSession userSession = this.SessionManager.Get(Guid.Parse(sessionId));
