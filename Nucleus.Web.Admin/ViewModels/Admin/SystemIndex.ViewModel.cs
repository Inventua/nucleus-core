...
public class SystemIndex
{
  ...
  public class LogEntry
  {
    ...
    public LogEntry(string input)
    {
      ...
      if (parts.Length >= 4)
      {
        string logEntryFormat = parts[0];
        int length = 0;
        DateTime logDate;

        switch (logEntryFormat)
        {
          // new style 
          case "A":
            ...
            this.Level = parts[2];
            length += this.Level.Length + 1;

            this.Category = parts[3];
            length += this.Category.Length + 1;

            this.IpAddress = parts[4];
            length += this.IpAddress.Length + 1;

            this.RequestPath = parts[5];
            length += this.RequestPath.Length + 1;

            break;

          default:
            // old-style
            if (DateTime.TryParse(parts[0], out logDate))
            {
        ...
        this.Message = input.Substring(length);
      }
      else
      {
