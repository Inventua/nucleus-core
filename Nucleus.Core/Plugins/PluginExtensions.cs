...
public static class PluginExtensions
{
  ...
  public static IMvcBuilder AddExternalControllers(this IMvcBuilder builder)
  {
    ...
    builder.Logger()?.LogInformation("Added ({count}) controllers from: {assemblies}].", externalControllerAssemblies.Count,
      String.Join(", ", externalControllerAssemblies.Select(assembly => $"{assembly.FullName} [{System.IO.Path.GetRelativePath(Environment.CurrentDirectory, assembly.Location)}]")));
  ...
  {
    List<Type> scheduledTaskTypes = new();
    ...
    foreach (Type type in AssemblyLoader.GetTypes<Nucleus.Abstractions.IScheduledTask>())
    {
      ...
      scheduledTaskTypes.Add(type);

      builder.Services.AddTransient(type);
    }

    builder.Logger()?.LogInformation("Added ({count}) scheduled task types: {assemblies}].", scheduledTaskTypes.Count,
      String.Join(", ", scheduledTaskTypes.Select(type => $"{type.FullName} from {type.Assembly.FullName} [{System.IO.Path.GetRelativePath(Environment.CurrentDirectory, type.Assembly.Location)}]")));
  ...
  {
    List<Type> portableImplementationTypes = new();
    ...
    foreach (Type type in AssemblyLoader.GetTypes<Nucleus.Abstractions.Portable.IPortable>())
    {
      ...
      portableImplementationTypes.Add(type);
      builder.Services.AddSingleton(typeof(Nucleus.Abstractions.Portable.IPortable), type);
    }

    builder.Logger()?.LogInformation("Added ({count}) IPortable implementations: {assemblies}].", portableImplementationTypes.Count,
      String.Join(", ", portableImplementationTypes.Select(type => $"{type.FullName} from {type.Assembly.FullName} [{System.IO.Path.GetRelativePath(Environment.CurrentDirectory, type.Assembly.Location)}]")));
