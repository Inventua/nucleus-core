...
namespace Nucleus.Core.DataProviders
{
	...
	public static class ModelExtensions
	{
		...
		/// </remarks>
		private static Dictionary<System.Type, IEnumerable<PropertyInfo>> ReflectionCache = new();
		...
		public static T Create<T>(IDataReader reader)  where T : new()
		{
			...
			if (!ReflectionCache.TryGetValue(typeof(T), out properties))
			{
				properties = BuildCacheEntry(typeof(T));
				ReflectionCache.Add(typeof(T), properties);
			}
			...
			foreach (PropertyInfo prop in properties)
			{
				if (DataHelper.FieldExists(reader, prop.Name))
				{
					...
					{
						System.Net.IPAddress tryValue;
						System.Net.IPAddress.TryParse(DataHelper.GetString(reader, prop.Name), out tryValue);
