...
public class Startup
{
  ...
  public void ConfigureServices(IServiceCollection services)
  {
    try
    {
      ...
      services.Logger().LogInformation("Platform: {platform}. Runtime Identifier: {rid}", platform, RuntimeInformation.RuntimeIdentifier);
      ...
      // in .NET8 this worked in Linux, but in .NET9, the IISServerOptions class can't be loaded in Linux (System.TypeLoadException), so
      // we have to wrap this in a check for Windows.
      // By moving the implementation to a separate function, we stop .NET from trying to load the IISServerOptions type, because the JIT
      // compiler does not load types until it needs to compile a method before it is called the first time, and AddIISServerOptions won't
      // be called if RuntimeInformation.IsOSPlatform(OSPlatform.Windows) is false.
      if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
      {
        services.AddIISServerOptions(Configuration);
        //services.Configure<IISServerOptions>(Configuration.GetSection("IISServerOptions"));

        // this doesn't cause a failure in Linux, but IISOptions aren't relevant in Linux 
      ...
      if (long.TryParse(maxRequestSize, out long maxRequestSizeValue))
      {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
          services.ConfigureIISRequestSize(maxRequestSizeValue);
      ...
      // For Linux only: None of the "default" behaviors work on Linux, so we have to persist keys to the file system.
